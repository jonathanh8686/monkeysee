...
const CUTOFF_ISO = "2026-01-09T23:59:59";
...
const Countdown = dynamic(
  () => import("./countdown").then((m) => m.Countdown),
  {
    ssr: false,
  }
);
...
export function HomeContent() {
  ...
  const session = DISABLE_AUTH
    ? { user: { name: "Test User", email: "test@example.com" } }
    : realSession;
  const isAuthLoading = DISABLE_AUTH ? false : realStatus === "loading";
  const user = session?.user;
  const isSignedIn = DISABLE_AUTH || !!user;
  ...
  const statsChanged = useCallback(
    (next: PredictionStats[], nextTotal: number) => {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!isSignedIn) {
      setFormError(
        DISABLE_AUTH
          ? "Auth is disabled, but somehow not signed in."
          : "Please sign in with Google before submitting a prediction."
      );
      return;
    }
  ...
  const textareaDisabled =
    isSubmitting || isClosed || !isSignedIn || isAuthLoading;
        ...
        <div className="w-full flex-1 space-y-8 lg:w-[65%] lg:flex lg:flex-col">
          <header className="flex items-start justify-between gap-4">
            <div className="space-y-2">
              ...
              {!isClosed && (
                <p className="text-[11px] font-mono text-zinc-400">
                  Closes on Jan 9 2026, 11:59pm · <Countdown target={cutoff} />
                </p>
              )}
            </div>
            ...
            <div className="flex flex-col items-end gap-2 text-xs">
              {DISABLE_AUTH ? (
                <span className="text-zinc-400">Auth disabled for testing</span>
              ) : isAuthLoading ? (
          ...
          <section className="space-y-4 rounded-3xl border border-zinc-800/60 bg-zinc-950/60 p-6 shadow-2xl shadow-black/40 backdrop-blur">
            <div className="flex items-center justify-between gap-4">
              ...
              {!isSignedIn && !isAuthLoading && !DISABLE_AUTH && (
            ...
            >
              <label className="block space-y-2">
                ...
                <textarea
                  ...
                  className="w-full min-h-24 resize-y rounded-xl border border-zinc-200 bg-white px-3 py-2 text-sm text-zinc-900 shadow-sm outline-none ring-0 transition focus:border-zinc-400 focus:ring-2 focus:ring-zinc-200 dark:border-zinc-800 dark:bg-zinc-950 dark:text-zinc-50 dark:focus:border-zinc-500 dark:focus:ring-zinc-800 disabled:bg-zinc-100/60 dark:disabled:bg-zinc-800/40"
